...
class GlyceBertForAblation(BertPreTrainedModel):
    def __init__(self, config):
        super(GlyceBertForAblation, self).__init__(config)
    ...
    def forward(
        ...
        **kwargs
        ...
        sequence_output = outputs[0]
        prediction_scores, pinyin_scores = self.cls(sequence_output)
        ...
        if pinyin_labels is not None:
            ...
            pinyin_loss = loss_fct(pinyin_scores.view(-1, 1378), active_labels)

        loss=None
        if masked_lm_loss is not None :
            loss=masked_lm_loss 
            if pinyin_loss is not None:
                loss+= pinyin_loss *gamma
        
        if not return_dict:
            output = (prediction_scores, pinyin_scores,) + outputs[2:]
            return ((loss,) + output) if loss is not None else output
...
class Decoupled_GlyceBertForMultiTask(BertPreTrainedModel):
    def __init__(self, config):
        super(Decoupled_GlyceBertForMultiTask, self).__init__(config)
...
class Attention_MultiTaskHeads(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = BertLMPredictionHead(config)
        self.Phonetic_relationship = Phonetic_Classifier(config)
        self.attention_c2p = Attention(config)
        self.attention_p2c = Attention(config)

    def forward(self, sequence_output, attention_mask=None,):
        hidden_char = self.predictions.transform(sequence_output)
        hidden_pinyin = self.Phonetic_relationship.transform(sequence_output)

        hidden_char = self.attention_p2c(hidden_pinyin, hidden_char, attention_mask= attention_mask)
        hidden_pinyin = self.attention_c2p(hidden_char, hidden_pinyin, attention_mask= attention_mask)

        prediction_scores = self.predictions.decoder(hidden_char)
        sm_scores = self.Phonetic_relationship.sm_classifier(hidden_pinyin)
        ym_scores = self.Phonetic_relationship.ym_classifier(hidden_pinyin)
        sd_scores = self.Phonetic_relationship.sd_classifier(hidden_pinyin)

        return prediction_scores, sm_scores,ym_scores,sd_scores


class Attention_GlyceBertForMultiTask(BertPreTrainedModel):
    def __init__(self, config):
...
class Dynamic_GlyceBertForMultiTask_with_KD(BertPreTrainedModel):
    def __init__(self, config):
        super(Dynamic_GlyceBertForMultiTask_with_KD, self).__init__(config)
