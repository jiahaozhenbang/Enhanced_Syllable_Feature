...
def create_instances_from_document(
    ...
    while i < len(document):
        ...
        if len(tokens) <= target_seq_length:
            item = create_masked_lm_predictions(
...
def create_masked_lm_predictions(tokens, masked_lm_prob, vocab_words, rng, text, encoded, tokenizer):
    ...
    tokens = encoded_input.tokens[1:-1]
    tokens_size = []
    for t in tokens:
        if t == '[UNK]':
    ...
    item['src_idx'] = input_ids
    assert len(item['tokens_size']) == len(item['src_idx']) - 2
    ...
    pinyin_label = token2pinyin.convert_sentence_to_shengmu_yunmu_shengdiao_ids(
        text, encoded)
    item['pinyin_label'] = pinyin_label

    assert len(input_ids) == len(pinyin_label), f'{text} --> {pinyin_label}'

    item['lengths'] = len(item['tokens_size'])
...
if __name__ == "__main__":
    main()
"""
python /home/ljh/CSC/Enhanced_Syllable_Feature/data_process/get_FPT_data_for_REALISE.py \
    --vocab_file /home/ljh/github/ReaLiSe-master/pretrained/vocab.txt \
    --input_data_dir /home/ljh/Data/wiki_zh --output_txt_dir /home/ljh/CSC/Enhanced_Syllable_Feature/data \
    --output_dir /home/ljh/github/ReaLiSe-master/data/FPT_SCOPE --adjust_confusionset
"""