...
class hanzi2pinyin():
    ...
    def __init__(self, chinese_bert_path, max_length: int = 512):
        ...
        self.pinyin_list = ['not chinese']
        for pronunciation in result:
            for py in pronunciation:
                if py != 'not chinese' and py not in self.pinyin_list:
                    self.pinyin_list.append(py)
    ...
    def convert_sentence_to_pinyin_label(self, sentence, tokenizer_output):
        # get pinyin of a sentence
        pinyin_list = pinyin(sentence, style=Style.TONE3, heteronym=True, errors=lambda x: [['not chinese'] for _ in x])
        pinyin_locs = {}
        # get pinyin of each location
        for index, item in enumerate(pinyin_list):
            pinyin_string = item[0]
            pinyin_locs[index] = self.pinyin_list.index(pinyin_string) if pinyin_string in self.pinyin_list else 0

        # find chinese character location, and generate pinyin ids
        pinyin_labels = []
        for idx, (token, offset) in enumerate(zip(tokenizer_output.tokens, tokenizer_output.offsets)):
            if offset[1] - offset[0] != 1:
                pinyin_labels.append(0)
                continue
            if offset[0] in pinyin_locs:
...
def all_data_no_dev_to_pickle(data_path, output_dir, vocab_path, max_len ):
    def _build_dataset(data_path):
        print('processing ', data_path)
        return build_dataset_with_tgt_pinyinids(
        data_path=data_path,
    ...
    write_data_to_txt(train_all, os.path.join(output_dir, 'train_all_no_dev_with_tgt_pinyinids'))
...
def build_dataset_with_tgt_pinyinids(data_path, vocab_path, max_len):
    # Load Data
    data_raw = []
    ...
    for item_raw in tqdm(data_raw, desc='Build Dataset'):
        ...
        item['pinyin_label'] = token2pinyin.convert_sentence_to_pinyin_label(item['tgt'], encoded)
...
"""
python /home/ljh/CSC/Enhanced_Syllable_Feature/data_process/dataset_for_ablation.py \
    --data_path /home/ljh/CSC/Enhanced_Syllable_Feature/data \
    --output_dir /home/ljh/CSC/Enhanced_Syllable_Feature/data/ablation_ft_data
"""
