...
from datasets.bert_csc_dataset import CSCDataset,TestCSCDataset, Dynaimic_CSCDataset,Dynaimic_CSCDataset_basedonLevenshtein
from models.modeling_multitask import GlyceBertForMultiTask,Attention_GlyceBertForMultiTask,Attention_and_weightLoss_GlyceBertForMultiTask, \
                                            Dynamic_GlyceBertForMultiTask,Levenshtein_GlyceBertForMultiTask
...
MODEL_MAP = {'ORINGIN': GlyceBertForMultiTask, 
        ...
        'Levenshtein': Levenshtein_GlyceBertForMultiTask,
        }
...
class CSCTask(pl.LightningModule):
    ...
    def forward(self, input_ids, pinyin_ids, labels=None, pinyin_labels=None, pron_similarity=None, var=1):
        """"""
        ...
        return self.model(
            ...
            labels=labels,
            pron_similarity=pron_similarity, 
            pinyin_labels=pinyin_labels,
            gamma=self.args.gamma if "gamma" in self.args else 0,
        )

    def compute_loss(self, batch):
        input_ids, pinyin_ids, labels, pron_similarity, pinyin_labels = batch
        loss_mask = (input_ids != 0) * (input_ids != 101) * (input_ids != 102).long()
        batch_size, length = input_ids.shape
        pinyin_ids = pinyin_ids.view(batch_size, length, 8)
        pron_similarity = pron_similarity.view(batch_size, length)
        outputs = self.forward(
            input_ids, pinyin_ids, labels=labels, pinyin_labels=pinyin_labels, pron_similarity=pron_similarity
        )
        loss = outputs.loss
        return loss
    ...
    def train_dataloader(self) -> DataLoader:
        name = "train_all_no_dev_with_Levenshtein_similarity"

        dataset = Dynaimic_CSCDataset_basedonLevenshtein(
    ...
    def get_dataloader(self, name="") -> DataLoader:
...
def get_parser():
    ...
    parser.add_argument("--model_architecture", choices=list(MODEL_MAP.keys()), default="Levenshtein", type=str,)
