...
def main():
    ...
    for index, cp in enumerate(cp_list):
        ...
        if '14' in args.label_file:
            output=trainer.predict(model=model,dataloaders=model.dev14_dataloader(),ckpt_path=args.ckpt_path)
        elif '13'in args.label_file:
            output=trainer.predict(model=model,dataloaders=model.dev13_dataloader(),ckpt_path=args.ckpt_path)
        else:
            output=trainer.predict(model=model,dataloaders=model.val_dataloader(),ckpt_path=args.ckpt_path)
        # print(output[:3])
        ...
        if results['sent-detect-f1'] >= max_df:
            max_df = results['sent-detect-f1']
